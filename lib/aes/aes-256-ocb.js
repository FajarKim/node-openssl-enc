...
/**
 * Encrypts data using the AES-256-OCB algorithm.
 *
...
function encryptAES256OCB(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("aes-256-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-256-OCB algorithm.
 *
 ...
 */
function decryptAES256OCB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("aes-256-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES256OCB, decryptAES256OCB };
