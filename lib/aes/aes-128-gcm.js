...
/**
 * Encrypts data using the AES-128-GCM algorithm.
 *
 ...
 */
function encryptAES128GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("aes-128-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the AES-128-GCM algorithm.
 *
 ...
 */
function decryptAES128GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("aes-128-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptAES128GCM, decryptAES128GCM };
