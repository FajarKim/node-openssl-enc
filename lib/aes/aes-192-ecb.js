...
/**
 * Encrypts data using the AES-192-ECB algorithm.
 *
 ...
 */
function encryptAES192ECB(data, password, extra = null) {
...
/**
 * Decrypts data using the AES-192-ECB algorithm.
 *
 ...
 */
function decryptAES192ECB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aes-192-ecb", key, null);
  const decrypted = Buffer.concat([decipher.update(data.slice(16)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptAES192ECB, decryptAES192ECB };
