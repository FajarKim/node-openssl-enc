...
/**
 * Encrypts data using the AES-256-CCM algorithm.
 *
 ...
 */
function encryptAES256CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("aes-256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-256-CCM algorithm.
 *
 ...
 */
function decryptAES256CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("aes-256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES256CCM, decryptAES256CCM };
