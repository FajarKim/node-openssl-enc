...
/**
 * Encrypts data using the AES-192-OCB algorithm.
 *
...
function encryptAES192OCB(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("aes-192-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-192-OCB algorithm.
 *
 ...
 */
function decryptAES192OCB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("aes-192-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES192OCB, decryptAES192OCB };
