...
/**
 * Encrypts data using the AES-192-OFB algorithm.
 *
...
function encryptAES192OFB(data, password, extra = null) {
...
/**
 * Decrypts data using the AES-192-OFB algorithm.
 *
 ...
 */
function decryptAES192OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aes-192-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptAES192OFB, decryptAES192OFB };
