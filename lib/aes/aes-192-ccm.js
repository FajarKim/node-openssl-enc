...
/**
 * Encrypts data using the AES-192-CCM algorithm.
 *
 ...
 */
function encryptAES192CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("aes-192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-192-CCM algorithm.
 *
 ...
 */
function decryptAES192CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("aes-192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES192CCM, decryptAES192CCM };
