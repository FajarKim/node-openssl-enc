...
/**
 * Encrypts data using the AES-128-CCM algorithm.
 *
...
function encryptAES128CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("aes-128-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-128-CCM algorithm.
 *
 ...
 */
function decryptAES128CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("aes-128-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES128CCM, decryptAES128CCM };
