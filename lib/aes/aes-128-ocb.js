...
/**
 * Encrypts data using the AES-128-OCB algorithm.
 *
 ...
 */
function encryptAES128OCB(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("aes-128-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the AES-128-OCB algorithm.
 *
 ...
 */
function decryptAES128OCB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("aes-128-ocb", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptAES128OCB, decryptAES128OCB };
