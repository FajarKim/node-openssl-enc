...
/**
 * Encrypts data using the AES-192-CFB8 algorithm.
 *
...
function encryptAES192CFB8(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("aes-192-cfb8", key, iv);
...
/**
 * Decrypts data using the AES-192-CFB8 algorithm.
 *
 ...
 */
function decryptAES192CFB8(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aes-192-cfb8", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptAES192CFB8, decryptAES192CFB8 };
