...
/**
 * Encrypts data using the ARIA-128-GCM algorithm.
 *
...
function encryptARIA128GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("aria-128-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ARIA-128-GCM algorithm.
 *
 ...
 */
function decryptARIA128GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("aria-128-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptARIA128GCM, decryptARIA128GCM };
