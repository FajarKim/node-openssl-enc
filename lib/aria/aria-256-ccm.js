...
/**
 * Encrypts data using the ARIA-256-CCM algorithm.
 *
...
function encryptARIA256CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("aria-256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the ARIA-256-CCM algorithm.
 *
 ...
 */
function decryptARIA256CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("aria-256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptARIA256CCM, decryptARIA256CCM };
