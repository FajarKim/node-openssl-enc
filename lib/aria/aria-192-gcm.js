...
/**
 * Encrypts data using the ARIA-192-GCM algorithm.
 *
 ...
 */
function encryptARIA192GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("aria-192-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ARIA-192-GCM algorithm.
 *
 ...
 */
function decryptARIA192GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("aria-192-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptARIA192GCM, decryptARIA192GCM };
