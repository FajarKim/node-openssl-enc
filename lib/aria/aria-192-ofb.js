...
/**
 * Encrypts data using the ARIA-192-OFB algorithm.
 *
 ...
 */
function encryptARIA192OFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("aria-192-ofb", key, iv);
...
/**
 * Decrypts data using the ARIA-192-OFB algorithm.
 *
 ...
 */
function decryptARIA192OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aria-192-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA192OFB, decryptARIA192OFB };
