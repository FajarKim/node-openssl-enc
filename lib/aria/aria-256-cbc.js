...
/**
 * Encrypts data using the ARIA-256-CBC algorithm.
 *
 ...
 */
function encryptARIA256CBC(data, password, extra = null) {
...
/**
 * Decrypts data using the ARIA-256-CBC algorithm.
 *
 ...
 */
function decryptARIA256CBC(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const decipher = crypto.createDecipheriv("aria-256-cbc", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA256CBC, decryptARIA256CBC };
