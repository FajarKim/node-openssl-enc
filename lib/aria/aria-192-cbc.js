...
/**
 * Encrypts data using the ARIA-192-CBC algorithm.
 *
...
function encryptARIA192CBC(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("aria-192-cbc", key, iv);
...
/**
 * Decrypts data using the ARIA-192-CBC algorithm.
 *
 ...
 */
function decryptARIA192CBC(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aria-192-cbc", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA192CBC, decryptARIA192CBC };
