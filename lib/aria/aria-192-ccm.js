...
/**
 * Encrypts data using the ARIA-192-CCM algorithm.
 *
 ...
 */
function encryptARIA192CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("aria-192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the ARIA-192-CCM algorithm.
 *
 ...
 */
function decryptARIA192CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("aria-192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptARIA192CCM, decryptARIA192CCM };
