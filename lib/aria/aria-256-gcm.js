...
/**
 * Encrypts data using the ARIA-256-GCM algorithm.
 *
...
function encryptARIA256GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("aria-256-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ARIA-256-GCM algorithm.
 *
 ...
 */
function decryptARIA256GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("aria-256-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptARIA256GCM, decryptARIA256GCM };
