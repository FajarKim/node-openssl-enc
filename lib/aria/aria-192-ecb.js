...
/**
 * Encrypts data using the ARIA-192-ECB algorithm.
 *
...
function encryptARIA192ECB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("aria-192-ecb", key, null);
...
/**
 * Decrypts data using the ARIA-192-ECB algorithm.
 *
 ...
 */
function decryptARIA192ECB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aria-192-ecb", key, null);
  const decrypted = Buffer.concat([decipher.update(data.slice(16)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA192ECB, decryptARIA192ECB };
