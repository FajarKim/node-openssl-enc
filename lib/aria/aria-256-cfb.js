...
/**
 * Encrypts data using the ARIA-256-CFB algorithm.
 *
 ...
 */
function encryptARIA256CFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const cipher = crypto.createCipheriv("aria-256-cfb", key, iv);
...
/**
 * Decrypts data using the ARIA-256-CFB algorithm.
 *
 ...
 */
function decryptARIA256CFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const decipher = crypto.createDecipheriv("aria-256-cfb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA256CFB, decryptARIA256CFB };
