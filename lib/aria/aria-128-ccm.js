...
/**
 * Encrypts data using the ARIA-128-CCM algorithm.
 *
...
function encryptARIA128CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("aria-128-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the ARIA-128-CCM algorithm.
 *
 ...
 */
function decryptARIA128CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("aria-128-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptARIA128CCM, decryptARIA128CCM };
