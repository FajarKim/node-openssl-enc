...
/**
 * Encrypts data using the ARIA-192-CFB1 algorithm.
 *
 ...
 */
function encryptARIA192CFB1(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("aria-192-cfb1", key, iv);
...
/**
 * Decrypts data using the ARIA-192-CFB1 algorithm.
 *
 ...
 */
function decryptARIA192CFB1(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("aria-192-cfb1", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA192CFB1, decryptARIA192CFB1 };
