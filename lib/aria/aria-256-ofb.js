...
/**
 * Encrypts data using the ARIA-256-OFB algorithm.
 *
...
function encryptARIA256OFB(data, password, extra = null) {
...
/**
 * Decrypts data using the ARIA-256-OFB algorithm.
 *
 ...
 */
function decryptARIA256OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const decipher = crypto.createDecipheriv("aria-256-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptARIA256OFB, decryptARIA256OFB };
