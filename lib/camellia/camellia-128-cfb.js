...
/**
 * Encrypts data using the CAMELLIA-128-CFB algorithm.
 *
...
function encryptCAMELLIA128CFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("camellia-128-cfb", key, iv);
...
/**
 * Decrypts data using the CAMELLIA-128-CFB algorithm.
 *
 ...
 */
function decryptCAMELLIA128CFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("camellia-128-cfb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA128CFB, decryptCAMELLIA128CFB };
