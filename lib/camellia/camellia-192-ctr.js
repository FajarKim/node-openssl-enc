...
/**
 * Encrypts data using the CAMELLIA-192-CTR algorithm.
 *
 ...
 */
function encryptCAMELLIA192CTR(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("camellia-192-ctr", key, iv);
...
/**
 * Decrypts data using the CAMELLIA-192-CTR algorithm.
 *
 ...
 */
function decryptCAMELLIA192CTR(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("camellia-192-ctr", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA192CTR, decryptCAMELLIA192CTR };
