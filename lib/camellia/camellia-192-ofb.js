...
/**
 * Encrypts data using the CAMELLIA-192-OFB algorithm.
 *
...
function encryptCAMELLIA192OFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("camellia-192-ofb", key, iv);
...
/**
 * Decrypts data using the CAMELLIA-192-OFB algorithm.
 *
 ...
 */
function decryptCAMELLIA192OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("camellia-192-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA192OFB, decryptCAMELLIA192OFB };
