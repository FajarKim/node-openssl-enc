...
/**
 * Encrypts data using the CAMELLIA-256-CBC algorithm.
 *
...
function encryptCAMELLIA256CBC(data, password, extra = null) {
...
/**
 * Decrypts data using the CAMELLIA-256-CBC algorithm.
 *
 ...
 */
function decryptCAMELLIA256CBC(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const decipher = crypto.createDecipheriv("camellia-256-cbc", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA256CBC, decryptCAMELLIA256CBC };
