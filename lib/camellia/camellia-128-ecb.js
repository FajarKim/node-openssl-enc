...
/**
 * Encrypts data using the CAMELLIA-128-ECB algorithm.
 *
 ...
 */
function encryptCAMELLIA128ECB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("camellia-128-ecb", key, null);
...
/**
 * Decrypts data using the CAMELLIA-128-ECB algorithm.
 *
 ...
 */
function decryptCAMELLIA128ECB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("camellia-128-ecb", key, null);
  const decrypted = Buffer.concat([decipher.update(data.slice(16)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA128ECB, decryptCAMELLIA128ECB };
