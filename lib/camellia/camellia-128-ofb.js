...
/**
 * Encrypts data using the CAMELLIA-128-OFB algorithm.
 *
 ...
 */
function encryptCAMELLIA128OFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("camellia-128-ofb", key, iv);
...
/**
 * Decrypts data using the CAMELLIA-128-OFB algorithm.
 *
 ...
 */
function decryptCAMELLIA128OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("camellia-128-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA128OFB, decryptCAMELLIA128OFB };
