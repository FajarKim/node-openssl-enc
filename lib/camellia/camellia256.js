...
/**
 * Encrypts data using the CAMELLIA256 algorithm.
 *
...
function encryptCAMELLIA256(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const cipher = crypto.createCipheriv("camellia256", key, iv);
...
/**
 * Decrypts data using the CAMELLIA256 algorithm.
 *
 ...
 */
function decryptCAMELLIA256(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha256");
  const decipher = crypto.createDecipheriv("camellia256", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptCAMELLIA256, decryptCAMELLIA256 };
