...
/**
 * Encrypts data using the SEED-OFB algorithm.
 *
 ...
 */
function encryptSEEDOFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("seed-ofb", key, iv);
...
/**
 * Decrypts data using the SEED-OFB algorithm.
 *
 ...
 */
function decryptSEEDOFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("seed-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptSEEDOFB, decryptSEEDOFB };
