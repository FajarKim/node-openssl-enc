...
/**
 * Encrypts data using the SEED algorithm.
 *
 ...
 */
function encryptSEED(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("seed", key, iv);
...
/**
 * Decrypts data using the SEED algorithm.
 *
 ...
 */
function decryptSEED(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("seed", key, iv);
