...
/**
 * Encrypts data using the ID-AES192-GCM algorithm.
 *
 ...
 */
function encryptIDAES192GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("id-aes192-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ID-AES192-GCM algorithm.
 *
 ...
 */
function decryptIDAES192GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("id-aes192-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptIDAES192GCM, decryptIDAES192GCM };
