...
/**
 * Encrypts data using the ID-AES192-CCM algorithm.
 *
...
function encryptIDAES192CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const cipher = crypto.createCipheriv("id-aes192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the ID-AES192-CCM algorithm.
 *
 ...
 */
function decryptIDAES192CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha512");
  const decipher = crypto.createDecipheriv("id-aes192-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptIDAES192CCM, decryptIDAES192CCM };
