...
/**
 * Encrypts data using the ID-AES256-CCM algorithm.
 *
...
function encryptIDAES256CCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("id-aes256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
/**
 * Decrypts data using the ID-AES256-CCM algorithm.
 *
 ...
 */
function decryptIDAES256CCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("id-aes256-ccm", key, Buffer.alloc(12), { authTagLength: 16 });
...
module.exports = { encryptIDAES256CCM, decryptIDAES256CCM };
