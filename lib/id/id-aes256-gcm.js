...
/**
 * Encrypts data using the ID-AES256-GCM algorithm.
 *
...
function encryptIDAES256GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const cipher = crypto.createCipheriv("id-aes256-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ID-AES256-GCM algorithm.
 *
 ...
 */
function decryptIDAES256GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 32, "sha512");
  const decipher = crypto.createDecipheriv("id-aes256-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptIDAES256GCM, decryptIDAES256GCM };
