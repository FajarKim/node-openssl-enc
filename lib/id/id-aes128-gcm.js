...
/**
 * Encrypts data using the ID-AES128-GCM algorithm.
 *
 ...
 */
function encryptIDAES128GCM(data, password, extra = null) {
  const salt = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const cipher = crypto.createCipheriv("id-aes128-gcm", key, Buffer.alloc(16));
...
/**
 * Decrypts data using the ID-AES128-GCM algorithm.
 *
 ...
 */
function decryptIDAES128GCM(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha512");
  const decipher = crypto.createDecipheriv("id-aes128-gcm", key, Buffer.alloc(16));
...
module.exports = { encryptIDAES128GCM, decryptIDAES128GCM };
