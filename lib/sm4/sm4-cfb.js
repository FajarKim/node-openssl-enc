...
/**
 * Encrypts data using the SM4-CFB algorithm.
 *
...
function encryptSM4CFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("sm4-cfb", key, iv);
...
/**
 * Decrypts data using the SM4-CFB algorithm.
 *
 ...
 */
function decryptSM4CFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("sm4-cfb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(32)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptSM4CFB, decryptSM4CFB };
