...
function encryptSM4(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(16);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("sm4", key, iv);
 ...
 */
function decryptSM4(encryptedData, password, extra = null) {
