...
/**
 * Encrypts data using the DES-EDE3-OFB algorithm.
 *
 ...
 */
function encryptDESEDE3OFB(data, password, extra = null) {
...
/**
 * Decrypts data using the DES-EDE3-OFB algorithm.
 *
 ...
 */
function decryptDESEDE3OFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("des-ede3-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESEDE3OFB, decryptDESEDE3OFB };
