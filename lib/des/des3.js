...
/**
 * Encrypts data using the DES3 algorithm.
 *
...
function encryptDES3(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("des3", key, iv);
...
/**
 * Decrypts data using the DES3 algorithm.
 *
 ...
 */
function decryptDES3(encryptedData, password, extra = null) {
