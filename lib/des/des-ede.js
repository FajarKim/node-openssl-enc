...
/**
 * Encrypts data using the DES-EDE algorithm.
 *
 ...
 */
function encryptDESEDE(data, password, extra = null) {
...
/**
 * Decrypts data using the DES-EDE algorithm.
 *
 ...
 */
function decryptDESEDE(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("des-ede", key, null);
  const decrypted = Buffer.concat([decipher.update(data.slice(16)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESEDE, decryptDESEDE };
