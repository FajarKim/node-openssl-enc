...
/**
 * Encrypts data using the DES-EDE-CFB algorithm.
 *
 ...
 */
function encryptDESEDECFB(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const cipher = crypto.createCipheriv("des-ede-cfb", key, iv);
...
/**
 * Decrypts data using the DES-EDE-CFB algorithm.
 *
 ...
 */
function decryptDESEDECFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 16, "sha256");
  const decipher = crypto.createDecipheriv("des-ede-cfb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESEDECFB, decryptDESEDECFB };
