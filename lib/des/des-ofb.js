...
/**
 * Encrypts data using the DES-OFB algorithm.
 *
...
function encryptDESOFB(data, password, extra = null) {
...
/**
 * Decrypts data using the DES-OFB algorithm.
 *
 ...
 */
function decryptDESOFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 8, "sha256");
  const decipher = crypto.createDecipheriv("des-ofb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESOFB, decryptDESOFB };
