...
/**
 * Encrypts data using the DES-CFB1 algorithm.
 *
 ...
 */
function encryptDESCFB1(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 8, "sha256");
  const cipher = crypto.createCipheriv("des-cfb1", key, iv);
...
/**
 * Decrypts data using the DES-CFB1 algorithm.
 *
 ...
 */
function decryptDESCFB1(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 8, "sha256");
  const decipher = crypto.createDecipheriv("des-cfb1", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESCFB1, decryptDESCFB1 };
