...
/**
 * Encrypts data using the DES-EDE3-CFB1 algorithm.
 *
...
function encryptDESEDE3CFB1(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("des-ede3-cfb1", key, iv);
...
/**
 * Decrypts data using the DES-EDE3-CFB1 algorithm.
 *
 ...
 */
function decryptDESEDE3CFB1(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("des-ede3-cfb1", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESEDE3CFB1, decryptDESEDE3CFB1 };
