...
/**
 * Encrypts data using the DESX algorithm.
 *
...
function encryptDESX(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("desx", key, iv);
...
/**
 * Decrypts data using the DESX algorithm.
 *
 ...
 */
function decryptDESX(encryptedData, password, extra = null) {
