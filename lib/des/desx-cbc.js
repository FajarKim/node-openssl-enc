...
/**
 * Encrypts data using the DESX-CBC algorithm.
 *
...
function encryptDESXCBC(data, password, extra = null) {
  const salt = crypto.randomBytes(8);
  const iv = crypto.randomBytes(8);
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const cipher = crypto.createCipheriv("desx-cbc", key, iv);
...
/**
 * Decrypts data using the DESX-CBC algorithm.
 *
 ...
 */
function decryptDESXCBC(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("desx-cbc", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESXCBC, decryptDESXCBC };
