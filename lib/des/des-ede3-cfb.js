...
/**
 * Encrypts data using the DES-EDE3-CFB algorithm.
 *
...
function encryptDESEDE3CFB(data, password, extra = null) {
...
/**
 * Decrypts data using the DES-EDE3-CFB algorithm.
 *
 ...
 */
function decryptDESEDE3CFB(encryptedData, password, extra = null) {
  ...
  const key = crypto.pbkdf2Sync(password, salt, 100000, 24, "sha256");
  const decipher = crypto.createDecipheriv("des-ede3-cfb", key, iv);
  const decrypted = Buffer.concat([decipher.update(data.slice(24)), decipher.final()]).toString("utf-8");

  return decrypted;
}

module.exports = { encryptDESEDE3CFB, decryptDESEDE3CFB };
